# @sittari/payload-rbac

> [!IMPORTANT]
> This is a localized fork of
> [`@whatworks/payload-rbac`](https://github.com/what-works-global/payload-packages/tree/main/packages/rbac),
> based on upstream `0.4.0` at commit `f7556471744fd5b01903cc2e452a0e22f41e618c`.
> See [FORK.md](./FORK.md) for provenance and divergence details.
...
The generated roles collection, roles field, permission options, and permissions
matrix follow the Payload admin language. English (`en`), Ukrainian (`uk`), and
Russian (`ru`) are included; unsupported language codes fall back to English.
Localized collection and global labels supplied by the host Payload config are
also preserved in the matrix.
...
```ts
import { hasPermission, requirePermission } from '@sittari/payload-rbac'

...
export const createProtectedRolesChangeHook = ({
  ...
  return ({ data, originalDoc, req }) => {
    ...
    const originalName =
      typeof originalDoc?.name === 'string' ? originalDoc.name : undefined;
    const incomingName = typeof data.name === 'string' ? data.name : undefined;
    const role =
      byName.get(originalName ?? '') ?? byName.get(incomingName ?? '');
    if (!role) {
      return data;
    }
    ...
    if (
      ...
      incomingName !== role.name
    ) {
      throw new APIError(
        `The role "${role.name}" is protected and cannot be renamed.`,
        403,
      );
    }
    if (
      'permissions' in data &&
      !samePermissions(data.permissions, role.permissions)
    ) {
      throw new APIError(
...
export const createProtectedRolesDeleteHook = ({
  ...
  return async ({ id, req }) => {
    ...
    if (name !== undefined && names.has(name)) {
      throw new APIError(
        `The role "${name}" is protected and cannot be deleted.`,
        403,
      );
    }
  };

...
export const rbacPlugin = (pluginConfig: RbacPluginConfig = {}): Plugin => {
  return (incomingConfig: Config): Config => {
    ...
    if (
      adminRole.name &&
      (pluginConfig.roles ?? []).some((role) => role.name === adminRole.name)
    ) {
      throw new Error(
        ...
        ? [
            {
              ...
              description:
                adminRole.description ?? 'Full access to everything.',
    ...
    if (
      (config.collections ?? []).some(
        (collection) => collection.slug === rolesCollectionSlug,
      )
    ) {
      throw new Error(
    ...
    const withCollectionAccess = (
      ...
      if (access.unlock === undefined) {
        access.unlock = createRbacAccess({
          slug: collection.slug,
          action: 'update',
        });
      }
    ...
    const withRolesField = (collection: CollectionConfig): CollectionConfig => {
      ...
      if (adminRole.name) {
        ...
        beforeChange.push(
          createProtectAdminUsersChangeHook(protectAdminUsersArgs),
        );
        beforeDelete.push(
          createProtectAdminUsersDeleteHook(protectAdminUsersArgs),
        );
      }
      ...
      if (adminRole.name) {
        ...
        beforeChange.push(
          createProtectLastAdminChangeHook(protectLastAdminArgs),
        );
        beforeDelete.push(
          createProtectLastAdminDeleteHook(protectLastAdminArgs),
        );
      }
      ...
      return {
        ...collection,
        fields,
        hooks: { ...collection.hooks, beforeChange, beforeDelete },
      };
    };

    const rolesAccess: CollectionConfig['access'] = {};
    for (const action of collectionActions) {
      rolesAccess[action] = createRbacAccess({
        slug: rolesCollectionSlug,
        action,
      });
    }
    ...
    const rolesBeforeChange: NonNullable<
      CollectionConfig['hooks']
    >['beforeChange'] = [];
    if (protectedRoles.length > 0) {
      rolesBeforeChange.push(
        createProtectedRolesChangeHook({ protectedRoles }),
      );
    }
    if (preventEscalation) {
      rolesBeforeChange.push(
        createProtectRolesCollectionHook({ protectedRoleNames }),
      );
    }
    const rolesHooks: CollectionConfig['hooks'] = {
      ...(rolesBeforeChange.length > 0
        ? { beforeChange: rolesBeforeChange }
        : {}),
    ...
    const lastUserCollectionIndex = mappedCollections.reduce(
      (last, collection, index) =>
        userCollections.includes(collection.slug) ? index : last,
      -1,
    );
    const insertAt =
      lastUserCollectionIndex === -1

...
describe('@sittari/payload-rbac peer smoke', () => {
  it('adds a roles collection with matrix options and the matrix field component', async () => {
    ...
    const values = permissionsField.options.map((o) =>
    ...
    const rows = (
      component as {
        clientProps: { rows: { actions: string[]; slug: string }[] };
      }
    ).clientProps.rows;
    ...
    expect(rows.find((r) => r.slug === 'site-settings')?.actions).toEqual([
      'read',
      'update',
    ]);
  ...
  it('injects the roles field into auth collections only', async () => {
    ...
    expect(posts.fields.some((f) => 'name' in f && f.name === 'roles')).toBe(
      false,
    );
  });
  ...
  it('resolves role IDs through one memoized find per request', async () => {
    const result = await rbacPlugin()(baseConfig());
    const find = vi.fn(() =>
      Promise.resolve({ docs: [role(7, ['posts:read'])] }),
    );
    const req = makeReq(
      result,
      { id: 1, collection: 'users', roles: [7] },
      { find },
    );
  ...
  it('blocks assigning roles whose permissions the assigner does not hold', async () => {
    ...
    const admin = makeReq(
      result,
      { id: 2, collection: 'users', roles: [role(2, ['*'])] },
      { find },
    );
    await expect(
      hook({
  ...
  it("blocks widening a role beyond the editing user's own permissions", async () => {
    ...
    const withProtected = createProtectRolesCollectionHook({
  ...
  it('repairs drifted permissions of protected roles on seed', async () => {
    ...
    const find = vi
      ...
      .mockResolvedValueOnce({
        docs: [{ id: 1, name: 'admin', permissions: ['posts:read'] }],
      })
      .mockResolvedValueOnce({
        docs: [{ id: 2, name: 'editor', permissions: ['posts:read'] }],
      });
    const payload = {
      create,
      db: { name: 'postgres' },
  ...
  it('locks the roles field to holders of roles:update', async () => {
    ...
    const adminExists = {
      ...
      find: vi.fn(() =>
        Promise.resolve({ docs: [{ id: 9, permissions: ['*'] }] }),
      ),
    };
    expect(
      await access({
  ...
  it('blocks removing roles from your own account that your kept roles cannot cover', async () => {
    ...
    const db = {
      ...
      find: vi.fn(
        (findArgs: { where?: { id?: { in?: (number | string)[] } } }) => {
          ...
          return Promise.resolve({
            docs: ids
              ? all.filter((r) => ids.map(String).includes(String(r.id)))
              : all,
          });
        },
      ),
    ...
    const self = {
      id: 7,
      collection: 'users',
      roles: [role(1, ['*']), role(2, ['posts:read'])],
    };
  ...
  it('locks every user’s credentials to the account owner by default, exempting only opted-out roles', async () => {
    ...
    const bare = await rbacPlugin({ preventPrivilegeEscalation: false })(
      baseConfig(),
    );
  ...
  it('keeps at least one user holding the admin role', async () => {
    const result = await rbacPlugin({ adminRole: 'Administrator' })(
      baseConfig(),
    );
    const args = {
  ...
  it('treats an adapter-wrapped unique violation as already-seeded (Mongo ValidationError)', async () => {
    ...
    const validationError = Object.assign(
      new Error('The following field is invalid: name'),
      {
        name: 'ValidationError',
        data: {
          collection: 'roles',
          errors: [{ message: 'Value must be unique', path: 'name' }],
        },
        status: 400,
      },
    );
  ...
  it('rejects invalid plugin configuration early', () => {
    expect(() =>
      rbacPlugin({
        roles: [{ name: 'admin', permissions: ['nonexistent:read'] }],
      })(baseConfig()),
  ...
  it('permission helpers match exact grants and full access', async () => {
    expect(permissionsGrant(new Set(['posts:read']), 'posts', 'read')).toBe(
      true,
    );
    expect(permissionsGrant(new Set(['posts:read']), 'posts', 'update')).toBe(
      false,
    );
    ...
    expect(
      missingPermissions(new Set(['posts:read']), ['posts:read', 'tags:read']),
    ).toEqual(['tags:read']);
  ...
  it('grants and covers wildcard permissions', async () => {
    ...
    expect(permissionsGrant(new Set(['posts:*']), 'posts', 'delete')).toBe(
      true,
    );
    expect(permissionsGrant(new Set(['posts:*']), 'pages', 'read')).toBe(false);
    expect(permissionsGrant(new Set(['*:read']), 'anything', 'read')).toBe(
      true,
    );
    expect(permissionsGrant(new Set(['*:read']), 'posts', 'update')).toBe(
      false,
    );
    ...
    expect(
      missingPermissions(new Set(['posts:*']), ['posts:read', 'posts:delete']),
    ).toEqual([]);
    expect(
      missingPermissions(new Set(['*:read']), ['posts:read', 'tags:read']),
    ).toEqual([]);
    expect(
      missingPermissions(
        new Set(['posts:create', 'posts:delete', 'posts:read', 'posts:update']),
        ['posts:*'],
      ),
    ).toEqual([]);
    expect(
      missingPermissions(new Set(['posts:read', 'tags:read']), ['*:read']),
    ).toEqual(['*:read']);
    ...
    expect(
      missingPermissions(new Set(everyActionWildcard), [
        '*',
        'posts:*',
        'tags:read',
      ]),
  ...
  it('accepts wildcard permissions in predefined roles, options, and access control', async () => {
    ...
    expect(() =>
      rbacPlugin({ roles: [{ name: 'X', permissions: ['nonexistent:*'] }] })(
        baseConfig(),
      ),
    ).toThrow(/unknown permission "nonexistent:\*"/);
    expect(() =>
      rbacPlugin({ roles: [{ name: 'X', permissions: ['*:publish'] }] })(
        baseConfig(),
      ),
  ...
  it('logs (does not throw) when existing duplicates block the unique index build', async () => {
    ...
    const payload = {
      db: {
        name: 'mongoose',
        collections: {
          roles: { createIndexes: () => Promise.reject(duplicateKey) },
        },
      },
      logger: { error },
    } as unknown as Payload;
      ...
      createIndexes(this: unknown) {
        if (this !== rolesModel) {
          return Promise.reject(
            new Error(
              '`Model.createIndexes()` cannot run without a model as `this`',
            ),
          );
        }
        return Promise.resolve();
  ...
  it('detects transient WriteConflict errors but not unique violations', () => {
    ...
    expect(
      isWriteConflict(
        Object.assign(new Error('x'), { code: 112, payloadInitError: true }),
      ),
    ).toBe(true);
    expect(isWriteConflict({ cause: { codeName: 'WriteConflict' } })).toBe(
      true,
    ); // wrapped
  ...
  it('detects the broader transient-error class but not permanent failures', () => {
    ...
    expect(isTransientMongoError({ name: 'MongoWriteConcernError' })).toBe(
      true,
    );
    expect(isTransientMongoError({ writeConcernError: { code: 11602 } })).toBe(
      true,
    );
    expect(
      isTransientMongoError({ codeName: 'InterruptedDueToReplStateChange' }),
    ).toBe(true);
    expect(
      isTransientMongoError({ errorLabels: ['RetryableWriteError'] }),
    ).toBe(true);
    expect(
      isTransientMongoError({ cause: { name: 'MongoWriteConcernError' } }),
    ).toBe(true); // wrapped
  ...
  it('retries write conflicts and rethrows every other error', async () => {
    ...
    await expect(retryOnWriteConflict(flaky, { delayMs: 0 })).resolves.toBe(
      'ok',
    );
    ...
    await expect(
      retryOnWriteConflict(stuck, { delayMs: 0, retries: 2 }),
    ...
    await expect(retryOnWriteConflict(failing, { delayMs: 0 })).rejects.toBe(
      boom,
    );

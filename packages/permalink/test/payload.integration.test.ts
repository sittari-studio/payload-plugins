...
describe('WordPress-style permalink integration', () => {
  ...
  it('suffixes conflicting published permalinks with the document id', async () => {
    ...
    expect(
      (await helpers.findDocumentByPath({ path: '/conflict-owner' }))?.document
        .id,
    ).toBe(owner.id);
    expect(
  ...
  it('does not suffix draft permalinks that overlap published routes', async () => {
    ...
    expect(
      (await helpers.findDocumentByPath({ path: '/draft-conflict' }))?.document
        .id,
    ).toBe(owner.id);
  ...
  it('does not move a published route when only its draft slug changes', async () => {
    ...
    expect(
      (await helpers.findDocumentByPath({ path: '/published-route' }))?.document
        .id,
    ).toBe(page.id);
    expect(
      await helpers.findDocumentByPath({ path: '/future-route' }),
    ).toBeNull();
  });
  ...
  it('moves the route when the changed draft is published', async () => {
    ...
    expect(
      await helpers.findDocumentByPath({ path: '/before-publish' }),
    ).toBeNull();
    expect(
      (await helpers.findDocumentByPath({ path: '/after-publish' }))?.document
        .id,
    ).toBe(page.id);
  });
  ...
  it('restores trashed documents as drafts without reclaiming occupied routes', async () => {
    ...
    expect(
      await helpers.findDocumentByPath({ path: '/restore-collision' }),
    ).toBeNull();
  ...
  it('keeps exact-document precedence over pagination fallback', async () => {
    ...
    const pageThree = await helpers.findDocumentByPath({
      path: '/blog/page/3',
    });

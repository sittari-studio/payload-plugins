import type { CollectionConfig, Config, Field, Plugin, UIField } from 'payload';
...
const normalizeSiteUrl = (siteUrl: unknown): string => {
  if (typeof siteUrl !== 'string' || siteUrl.trim().length === 0) {
    throw new Error(
      '@sittari/payload-permalink: siteUrl must be a non-empty URL.',
    );
  }
  const parsed = new URL(siteUrl);
  if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
    throw new Error(
      '@sittari/payload-permalink: siteUrl must use http or https.',
    );
  }
...
const normalizeConfig = (
  ...
  if (!isRecord(pluginConfig.collections)) {
    throw new Error(
      '@sittari/payload-permalink: collections must be a mapping.',
    );
  }
...
export const permalinkPlugin = (
  ...
  const plugin: Plugin = (incomingConfig) => {
    ...
    for (const [slug, options] of Object.entries(collections)) {
      ...
      const hideRestoreAsPublished = Boolean(
        collection.trash && collectionHasDrafts,
      );
      const useAsSlug =

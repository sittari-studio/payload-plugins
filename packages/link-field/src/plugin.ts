...
const getCollectionLabel = (collection: CollectionConfig): StaticLabel =>
  getStaticLabel(
    (collection.labels as { singular?: unknown } | undefined)?.singular,
  ) ?? collection.slug;

const getCollectionUseAsTitle = (
...
const getApiRoute = (config: Config): string =>
  getStringValue((config.routes as { api?: unknown } | undefined)?.api) ??
  '/api';
...
export const linkFieldPlugin =
  ...
  (incomingConfig: Config): Config => {
    ...
    const collectionSummaries = getReferenceSummaryCollections(
    ...
    const finalConfig: Config = {
      ...
      collections: incomingConfig.collections?.map((collection) => ({
        ...
        fields:
          traverseFields(
            collection.fields,
            createTransformField(collection.slug),
          ) ?? [],
      })),

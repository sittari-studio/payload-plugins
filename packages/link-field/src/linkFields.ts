import type { CollectionSlug, Field, RelationshipField, TextField } from 'payload'
...
export const getReferenceDocumentId = (value: unknown): unknown => {
  ...
  if (!reference.value || typeof reference.value !== 'object') return reference.value
...
export const createLinkFields = ({
  ...
  return [
    ...
    ...(showLabel
      ? [{ name: 'label', type: 'text', label: localizedText('label') } satisfies TextField]
      : []),
    customUrlField,
    referenceField,
    ...(showNewTab
      ? [{ name: 'newTab', type: 'checkbox', label: localizedText('openInNewTab') } as const]
      : []),
    {
      name: 'url',
      type: 'text',

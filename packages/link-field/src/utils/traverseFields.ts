...
const transformBlock = (block: Block, transformer: FieldTransformer): Block => ({
  ...block,
  fields: block.fields.map((field) => traverseField(field, transformer)),
})

export const traverseField = (field: Field, transformer: FieldTransformer): Field => {
  let nextField = field

  if (hasFields(nextField)) {
    nextField = {
      ...nextField,
      fields: nextField.fields.map((childField) => traverseField(childField, transformer)),
    } as Field
  }
...
): Field[] | undefined => fields?.map((field) => traverseField(field, transformer))

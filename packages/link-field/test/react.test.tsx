...
describe('LinkFieldJSXConverter', () => {
  it('renders custom and reference links only from their populated URLs', () => {
    expect(
      renderNode(linkNode({ fields: { customUrl: '/ignored', type: 'custom', url: '/about' } })),
    ).toBe('<a href="/about">Example</a>')
    expect(
      renderNode(linkNode({ fields: { reference: 1, type: 'reference', url: '/posts/one' } })),
  ...
  it('supports autolinks and new-tab attributes', () => {
    expect(
      renderNode(
        linkNode({
          fields: { customUrl: 'https://example.com', newTab: true, type: 'custom', url: 'https://example.com' },
          nodeType: 'autolink',
        }),
      ),
    ).toBe(

...
describe('templatesPlugin', () => {
  ...
  it('returns the incoming config unchanged when disabled', () => {
    ...
    expect(
      templatesPlugin({ enabled: false, templates: definitions })(input),
    ).toBe(input);
  });
  ...
  it.each([
    ...
    {
      templates: [
        { name: 'page-404', label: 'Invalid characters', fields: [] },
      ],
      message: 'Invalid template name',
    },
    {
  ...
  it('expands template fields immutably in collections, globals, and blocks', () => {
    ...
    const globalContent = getNamedField(
      output.globals?.[0]?.fields ?? [],
      'content',
    );
    const blockContent = getNamedField(
      output.blocks?.[0]?.fields ?? [],
      'blockContent',
    );
    ...
    for (const content of [collectionContent, globalContent, blockContent]) {
      ...
      expect(getNamedField(nested.fields, 'caption')).toMatchObject({
        required: false,
      });
      ...
      expect(getNamedField(referencedBlock.fields, 'body')).toMatchObject({
        required: false,
      });
  ...
  it('adds template values as placeholders without changing structural fields', () => {
    ...
    expect(
      getNamedField(nested.fields as Field[], 'description'),
    ).toMatchObject({
    ...
    expect(
      getNamedField(tabs.tabs[0].fields as Field[], 'title'),
    ).toMatchObject({
  ...
  it('reconciles once on the first collection operation after config hot reload', async () => {
    const create = vi.fn(async () => undefined);
    const find = vi.fn(
      async (_options?: { context?: Record<string, unknown> }) => ({
        docs: [],
      }),
    );
    const payload = {
...
describe('createTemplateGetter', () => {
  ...
  it('finds by templateType and selects only the matching group field', async () => {
    ...
    expectTypeOf(document).toEqualTypeOf<Pick<
      GeneratedTemplate,
      'id' | 'data_404'
    > | null>();
  });

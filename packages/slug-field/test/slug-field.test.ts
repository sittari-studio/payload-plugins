...
describe('createSlugField', () => {
  ...
  it('generates an empty slug before required validation', async () => {
    ...
    if (!slug || slug.type !== 'text') throw new Error('Expected a text slug field')
    ...
    } as unknown as Parameters<typeof beforeValidate>[0])).toBe('a-new-page')
  })
  ...
  it('preserves a manually entered slug before validation', async () => {
    ...
    expect(beforeValidate({
      value: 'custom-path',
      siblingData: { title: 'A New Page' },
    } as unknown as Parameters<typeof beforeValidate>[0])).toBe('custom-path')
  })
  ...
  it('leaves the slug unset when its source is empty', async () => {
    ...
    if (!generateSlug || generateSlug.type !== 'checkbox') throw new Error('Expected a slug generation checkbox')

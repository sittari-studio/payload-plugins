...
export const resolveNextMetadata = async (
  input:
    | {
        payload: SeoPayload;
        collection: string;
  ...
  if (metadata.canonicalUrl || metadata.alternates)
    result.alternates = {
      ...(metadata.canonicalUrl ? { canonical: metadata.canonicalUrl } : {}),
      ...(metadata.alternates ? { languages: metadata.alternates } : {}),
    };
  if (metadata.robots) {
    result.robots = metadata.robots.custom?.length
      ...
      : {
          ...(metadata.robots.index
            ? { index: metadata.robots.index === 'index' }
            : {}),
          ...(metadata.robots.follow
            ? { follow: metadata.robots.follow === 'follow' }
            : {}),
        };
  }

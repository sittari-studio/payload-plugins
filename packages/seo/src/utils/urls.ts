...
export const normalizeCanonicalUrl = (value: unknown, policy: TrailingSlashPolicy = 'never'): string | undefined => {
  if (!isAbsoluteHttpUrl(value)) return undefined
  try {
    const url = new URL(value.trim())
    if (url.search || url.hash) return undefined
    if (url.pathname !== '/') {
      url.pathname = policy === 'always'
...
export const combineSiteUrl = (siteUrl: unknown, path: unknown, policy: TrailingSlashPolicy = 'never'): string | undefined => {

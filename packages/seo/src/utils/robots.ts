...
const simple = new Set(['noarchive', 'nosnippet', 'noimageindex', 'notranslate'])
...
export const normalizeRobotsDirectives = (value: unknown): string[] => {
  ...
  const add = (candidate: string): void => {
    ...
    const supported = simple.has(directive)
  ...
  if (Array.isArray(value)) value.forEach((item) => typeof item === 'string' && item.split(',').forEach(add))
  else nonEmptyString(value)?.split(',').forEach(add)
  return directives
}

export const robotsContent = (robots: { index?: 'index' | 'noindex'; follow?: 'follow' | 'nofollow'; custom?: string[] }): string =>
  [robots.index, robots.follow, ...(robots.custom ?? []).filter((value) => !indexFollow.has(value))]

...
export const normalizeRedirectPath = (value: unknown): string | null => {
  ...
  try {
    ...
    if (url.origin !== 'https://payload-seo.invalid' || url.search || url.hash)
      return null;
    return path;
  } catch {
    return null;
...
const validationText = (
  key: 'validationAbsoluteHttpUrl' | 'validationJson',
  context?: ValidationContext,
): string => adminText(key, context?.req?.i18n?.language);
...
export const validateCanonicalUrl = (
  ...
  value === null ||
  value === '' ||
  normalizeCanonicalUrl(value, policy)
    ? true
    : validationText('validationAbsoluteHttpUrl', context);
...
export const validateJson = (
  ...
  try {
    return isPlainJsonObject(JSON.parse(value))
      ? true
      : validationText('validationJson', context);
  } catch {
    return validationText('validationJson', context);
  }
};

export const hasLineBreak = (value: unknown): boolean =>
  typeof value === 'string' && /[\r\n]/.test(value);

export const validateRobotsToken = (
  value: unknown,
  context?: ValidationContext,
): true | string =>
  value === undefined ||
  value === null ||
  value === '' ||
  (typeof value === 'string' && !hasLineBreak(value))
    ? true
    : validationText('validationAbsoluteHttpUrl', context);

...
const settingsTemplates = (
  ...
  const globals = Array.isArray(settings.globalSchemas)
  ...
  const group = groups.find(
    (item) =>
      item &&
      typeof item === 'object' &&
      (item as Record<string, unknown>).collection === collection,
...
const fieldCanRead = async (
  ...
  if (
    !field ||
    !('access' in field) ||
    typeof field.access?.read !== 'function'
  )
    return true;
...
export const createSchemaTemplatesEndpoint = ({
  collection,
  seoField,
  settingsGlobal,
}: {
  ...
  handler: async (req) => {
    ...
    if (!localeIsConfigured(req, requestedLocale))
    ...
    try {
      ...
      return Response.json({
        ...settingsTemplates(settings, collection),
        defaultLocale: defaultLocale(req),
      });
    } catch {
      return Response.json({ message: 'Forbidden' }, { status: 403 });
    }

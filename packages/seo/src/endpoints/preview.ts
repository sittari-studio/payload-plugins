...
const merge = (base: SeoDocument, overlay: SeoDocument): SeoDocument => Object.entries(overlay).reduce<SeoDocument>((result, [key, value]) => {
  const inherited = result[key]
  result[key] = isPlainJsonObject(inherited) && isPlainJsonObject(value) ? merge(inherited, value) : value
  return result
}, { ...base })
...
const localeIsConfigured = (payload: SeoPayload, locale: string): boolean => {
  ...
  return configured.some((item) => (typeof item === 'string' ? item : item.code) === locale)
}

const hasPreviewAccess = async (req: any, collection: string, seoField: string, document?: SeoDocument): Promise<boolean> => {
  const config = req.payload.config?.collections?.find((candidate: any) => candidate.slug === collection)
  ...
  if (typeof fieldAccess === 'function' && await fieldAccess({ req, doc: document, data: document, siblingData: document }) !== true) return false
  return true
}
...
export const createSeoPreviewEndpoint = (collection: string): Omit<Endpoint, 'root'> => ({
  ...
  handler: async (req) => {
    ...
    if (!document || locale === undefined || !localeIsConfigured(req.payload as unknown as SeoPayload, locale)) return Response.json({ message: 'Invalid preview request.' }, { status: 400 })

    const config = getSeoRuntimeConfig(req.payload as unknown as SeoPayload)
    if (!config?.collections[collection]) return Response.json({ message: 'Not found' }, { status: 404 })
    const names = resolveSeoNames(config.names)
    const id = document.id
    if (id !== undefined && typeof id !== 'string' && typeof id !== 'number') return Response.json({ message: 'Invalid preview request.' }, { status: 400 })
    ...
    if (id !== undefined) {
      try {
        ...
        persisted = await (req.payload as unknown as SeoPayload).findByID({ collection, id, locale, fallbackLocale: false, draft: true, depth: 0, overrideAccess: false, req, user: req.user })
      } catch {
        return Response.json({ message: 'Forbidden' }, { status: 403 })
      }
    }

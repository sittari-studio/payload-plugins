  ...
  },
  {
    ...
    validate: (
      ...
      validateJsonPatch(value, {
        ...
        source: siblingData?.schema as JsonObject | undefined,
      }) === true
        ? true
        : adminText('validationSchemaPatch', req?.i18n?.language),
  },
  ...(mode === 'collection'
    ? [{ name: 'isDefault', type: 'checkbox' as const, defaultValue: false }]
    : []),
];
...
const allTemplateIds = (document: unknown): string[] => {
  ...
  if (Array.isArray(record.collectionSchemas))
    for (const group of record.collectionSchemas) {
      if (group && typeof group === 'object')
        collect((group as Record<string, unknown>).templates);
    }
  return ids;
};
...
const enforceUniqueTemplateIds: GlobalBeforeChangeHook = ({ data }) => {
  ...
  for (const id of allTemplateIds(data)) {
    if (seen.has(id))
      throw new Error(
        `Schema template ID "${id}" must be unique across global and collection schemas.`,
      );
    seen.add(id);
  }
  return data;
...
const cascadeDeletedTemplates =
  (seoField: string, collections: string[]): GlobalBeforeChangeHook =>
  async ({ data, originalDoc, req }) => {
    const previous = templateIdsByScope(originalDoc);
    const next = templateIdsByScope(data);
    const deletedCollection = new Set(
      [...previous.collection].filter((id) => !next.collection.has(id)),
    );
    const deletedGlobal = new Set(
      [...previous.global].filter((id) => !next.global.has(id)),
    );
    if (!deletedCollection.size && !deletedGlobal.size) return data;

    for (const collection of collections) {
      const clauses: Where[] = [];
      if (deletedCollection.size)
        clauses.push({
          [`${seoField}.schemaInstances.templateId`]: {
            in: [...deletedCollection],
          },
        });
      if (deletedGlobal.size)
        clauses.push({
          [`${seoField}.globalSchemaOverrides.schemaId`]: {
            in: [...deletedGlobal],
          },
        });
      ...
      for (const document of result.docs) {
        const seo =
          ...
          (document as Record<string, unknown>)[seoField];
        const current =
        ...
        const schemaInstances = Array.isArray(current.schemaInstances)
          ? current.schemaInstances.filter(
              (item) =>
                !item ||
                typeof item !== 'object' ||
                !deletedCollection.has(
                  String((item as Record<string, unknown>).templateId),
                ),
            )
          : [];
        const globalSchemaOverrides = Array.isArray(
          current.globalSchemaOverrides,
        )
          ? current.globalSchemaOverrides.filter(
              (item) =>
                !item ||
                typeof item !== 'object' ||
                !deletedGlobal.has(
                  String((item as Record<string, unknown>).schemaId),
                ),
            )
          : [];
...
export const createSeoSettingsGlobal = ({
  access,
  slug,
  mediaCollection,
  collectionVariables,
  ...
  access?: SeoPluginAccess['settings'];
  slug: string;
  ...
  hooks: {
    beforeChange: [
      enforceUniqueTemplateIds,
      cascadeDeletedTemplates(seoField, Object.keys(collectionVariables)),
    ],
  },
  admin: {
          ...
          fields: [
            {
              name: 'siteName',
              type: 'text',
              label: adminLabel('siteName'),
              localized: true,
            },
            {
              name: 'titleTemplate',
              type: 'text',
              label: adminLabel('titleTemplate'),
              localized: true,
            },
            {
              name: 'defaultDescription',
              type: 'textarea',
              label: adminLabel('defaultDescription'),
              localized: true,
            },
            {
              name: 'defaultKeywords',
              type: 'text',
              label: adminLabel('defaultKeywords'),
              localized: true,
              admin: { description: adminTabLabel('keywordsDescription') },
            },
          ],
        },
        {
          ...
          fields: [
            {
              ...
              type: 'upload',
              label: adminLabel('defaultOpenGraphImage'),
            ...
            },
            {
              ...
              type: 'select',
              label: adminLabel('defaultTwitterCard'),
              localized: true,
              options: [
                { label: adminLabel('summary'), value: 'summary' },
                {
                  label: adminLabel('summaryLargeImage'),
                  value: 'summary_large_image',
                },
              ],
            },
            {
              name: 'defaultOpenGraphType',
              type: 'text',
              label: adminLabel('defaultOpenGraphType'),
              localized: true,
            },
            {
              name: 'defaultTwitterSite',
              type: 'text',
              label: adminLabel('defaultTwitterSite'),
              localized: true,
            },
            {
              name: 'defaultTwitterCreator',
              type: 'text',
              label: adminLabel('defaultTwitterCreator'),
              localized: true,
            },
            {
              name: 'defaultLocale',
              type: 'text',
              label: adminLabel('defaultLocale'),
              localized: true,
            },
          ],
        },
          ...
          fields: [
            {
              name: 'defaultRobots',
              type: 'group',
              label: adminLabel('defaultRobots'),
              localized: true,
              fields: [
                {
                  name: 'mode',
                  type: 'select',
                  label: adminLabel('robotsMode'),
                  required: true,
                  defaultValue: 'index-follow',
                  options: [
                    { label: adminLabel('indexFollow'), value: 'index-follow' },
                    {
                      label: adminLabel('noindexFollow'),
                      value: 'noindex-follow',
                    },
                    {
                      label: adminLabel('indexNofollow'),
                      value: 'index-nofollow',
                    },
                    {
                      label: adminLabel('noindexNofollow'),
                      value: 'noindex-nofollow',
                    },
                    { label: adminLabel('customDirectives'), value: 'custom' },
                  ],
                },
                {
                  name: 'directives',
                  type: 'text',
                  label: adminLabel('directives'),
                  admin: {
          ...
          fields: [
            {
              ...
              type: 'array',
              admin: { hidden: true },
              fields: schemaTemplateFields('global'),
            },
            {
              ...
              fields: [
                {
                  ...
                  type: 'select',
                  required: true,
                  options: Object.keys(collectionVariables),
                },
                {
                  name: 'templates',
                  type: 'array',
                  fields: schemaTemplateFields('collection'),
                },
              ],
              admin: { hidden: true },
            },
            {
              ...
              admin: {
                components: {
                  Field: SEO_SETTINGS_SCHEMA_MANAGER_ADMIN_COMPONENT,
                },
                custom: {
                  seo: {
                    mode: 'settings',
                    collections: Object.keys(collectionVariables),
                    collectionVariables,
                    defaultLocale,
                    globalVariables,
                    labeledCollections,
              ...
              fields: [
                {
                  ...
                  options: [
                    { label: adminLabel('generated'), value: 'generated' },
                    { label: adminLabel('override'), value: 'override' },
                  ],
                },
                {
                  name: 'groups',
                  type: 'array',
                  label: adminLabel('groups'),
                  fields: [
                    {
                      name: 'userAgent',
                      type: 'text',
                      label: adminLabel('userAgent'),
                      required: true,
                      validate: validateRobotsToken,
                    },
                    {
                      name: 'allow',
                      type: 'array',
                      label: adminLabel('allow'),
                      fields: [
                        {
                          name: 'path',
                          type: 'text',
                          label: adminLabel('path'),
                          validate: validateRobotsToken,
                        },
                      ],
                    },
                ...
                },
                {
                  name: 'overrideText',
                  type: 'textarea',
                  label: adminLabel('overrideText'),
                  admin: {
                    condition: (_, siblingData) =>
                      siblingData?.mode === 'override',
                  },
                },
              ],
            },

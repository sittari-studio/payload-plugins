...
export const containsReservedSchemaKey = (value: unknown): boolean => {
  ...
  return Object.entries(value).some(
    ([key, child]) => key === '@context' || containsReservedSchemaKey(child),
  );
};

export const validateSchemaObject = (value: unknown): true | string => {
  if (!isJsonObject(value))
    return 'Schema JSON must have an object at its root.';
  if (containsReservedSchemaKey(value))
    return 'Schema JSON must not contain the reserved @context key.';
  return true;
};
...
export const validateJsonPatch = (
  value: unknown,
  options: { scalarValuesOnly?: boolean; source?: JsonObject } = {},
): true | string => {
  if (value === undefined || value === null) return true;
  if (!Array.isArray(value))
    return 'Schema overrides must be a JSON Patch array.';
  for (const operation of value) {
    if (
      !isJsonObject(operation) ||
      !['add', 'replace', 'remove'].includes(String(operation.op))
    )
      return 'Only add, replace, and remove patch operations are supported.';
    const segments =
      typeof operation.path === 'string'
        ? parseJsonPointer(operation.path)
        : null;
    if (!segments)
      return 'Every patch operation requires a valid JSON Pointer path.';
    if (segments.includes('@context'))
      return 'Schema overrides must not target the reserved @context key.';
    if (operation.op !== 'remove' && !Object.hasOwn(operation, 'value'))
      return 'Add and replace patch operations require a value.';
    if (operation.op !== 'remove' && containsReservedSchemaKey(operation.value))
      return 'Schema overrides must not add the reserved @context key.';
    if (options.scalarValuesOnly) {
      if (operation.op !== 'replace')
        return 'Localized template overrides may only replace existing scalar values.';
      const replacementType = scalarType(operation.value);
      if (!replacementType)
        return 'Localized template overrides may only replace existing scalar values.';
      if (options.source) {
        const target = valueAtPointer(options.source, segments);
        if (!target.exists || scalarType(target.value) !== replacementType)
          return 'Localized template overrides must preserve the type of an existing scalar value.';
      }
    }
  }
  return true;
...
export const applyJsonPatch = (
  source: JsonObject,
  patch: readonly SeoJsonPatchOperation[] = [],
): JsonObject => {
  ...
  for (const operation of patch) {
    ...
    if (!segments.length) {
      if (operation.op === 'remove')
        throw new Error('The schema root cannot be removed.');
      if (!isJsonObject(operation.value))
        throw new Error('The schema root must remain an object.');
    ...
    for (const segment of segments.slice(0, -1)) {
      if (Array.isArray(parent)) {
        ...
        if (index === null || parent[index] === undefined)
    ...
    if (Array.isArray(parent)) {
      ...
      if (index === null || (operation.op !== 'add' && index >= parent.length))
        throw new Error(`Patch path does not exist: ${operation.path}`);
      if (operation.op === 'add')

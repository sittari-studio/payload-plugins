...
const excluded = (path: string, prefixes: readonly string[]): boolean =>
...
export const discoverSchemaVariables = ({
  ...
  fields,
  exclusions = [],
  generatedSeoField = 'seo',
}: {
  collection: string;
  fields: Field[];
  exclusions?: readonly string[];
  generatedSeoField?: string;
}): SeoSchemaVariable[] => {
    ...
    for (const field of items) {
      ...
      if (field.type !== 'group') {
        result.push({
          collection,
          path,
          label: labelText(field.label, field.name),
        });
      }
...
export const groupSchemaVariables = (
  collections: Record<string, SeoSchemaVariable[]>,
): SeoSchemaVariable[] => {
  const collectionCount = Object.keys(collections).length;
  const occurrences = new Map<string, number>();
  for (const variables of Object.values(collections))
    for (const path of new Set(variables.map((item) => item.path)))
      occurrences.set(path, (occurrences.get(path) ?? 0) + 1);
  return Object.values(collections)
    .flat()
    .map((item) => ({
      ...item,
      availableInEveryCollection:
        occurrences.get(item.path) === collectionCount,
    }));
};
...
const resolveString = (
  ...
  let missing = false;
  const resolved = escaped.replace(
    /\$([A-Za-z0-9_.-]+)/g,
    (_, path: string) => {
...
export const substituteSchemaVariables = (
  value: unknown,
  document: Record<string, unknown>,
): unknown => {

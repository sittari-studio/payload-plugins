...
export const findSeoRedirect = async ({
  payload,
  sourcePath,
}: {
  payload: SeoPayload;
  ...
  try {
    const result = await payload.find({
      ...
      where: {
        and: [{ source: { equals: source } }, { enabled: { equals: true } }],
      },
    });
    const redirect = result.docs[0] ?? {};
    const statusCode =
      redirect.statusCode === 301 || redirect.statusCode === '301'
        ? 301
        : redirect.statusCode === 302 || redirect.statusCode === '302'
          ? 302
          : undefined;
    const destination =
      redirect.destinationType === 'internal'
        ? normalizeRedirectPath(redirect.destination)
        : isAbsoluteHttpUrl(redirect.destination)

...
const setPath = (document: SeoDocument, path: string, value: unknown): void => {
  ...
  for (let index = 0; index < segments.length - 1; index++) {
    ...
    if (Array.isArray(target)) {
      ...
      if (!current || typeof current !== 'object' || Array.isArray(current) !== nextIsArray) target[key] = nextIsArray ? [] : {}
      target = target[key] as Record<string, unknown> | unknown[]
    } else {
      const current = target[segment]
      if (!current || typeof current !== 'object' || Array.isArray(current) !== nextIsArray) target[segment] = nextIsArray ? [] : {}
...
export const previewDocumentFromForm = (savedDocument: unknown, fields: PreviewFormFields): SeoDocument => {
  const document = savedDocument !== null && typeof savedDocument === 'object' && !Array.isArray(savedDocument)

...
import {
  payloadArrayRowState,
  type NestedArrayOptions,
} from './payloadArrayState.js';
...
export const usePayloadArray = <Row extends object>({
  ...
  useFormFields(([fields]) =>
    JSON.stringify(
      Object.entries(fields).flatMap(([fieldPath, state]) =>
        fieldPath === path || fieldPath.startsWith(`${path}.`)
          ? [[fieldPath, state]]
          : [],
      ),
    ),

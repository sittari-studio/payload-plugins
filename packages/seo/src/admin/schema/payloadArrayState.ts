...
export type NestedArrayOptions = Record<string, { nestedArrays?: NestedArrayOptions }>
...
const generatedRowID = (): string =>
  globalThis.crypto?.randomUUID?.() ?? `seo-${Date.now()}-${Math.random().toString(36).slice(2)}`
...
export const payloadArrayRowState = (
  ...
  for (const [name, value] of Object.entries({ ...row, id: rowID })) {
    ...
    const rows = value.map((item) => {
      const nestedRow = item && typeof item === 'object' && !Array.isArray(item)
        ? item as Record<string, unknown>
        : {}
      const id = typeof nestedRow.id === 'string' ? nestedRow.id : generatedRowID()
      return { id }
    })
    state[name] = {

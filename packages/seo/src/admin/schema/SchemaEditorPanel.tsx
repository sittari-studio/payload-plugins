...
export const SchemaEditorPanel = ({
  ...
  return (
    <div className="st-grid st-gap-base [&_.checkbox-input]:st-mb-0 [&_.field-type]:st-mb-0 [&_p]:st-mb-0 [&_p]:st-mt-[.35rem] [&_p]:st-text-elevation-600">
      {onReplace && !structuralLocked && !readOnly ? (
        <nav
          className="st-border-0 st-border-b st-border-solid st-border-elevation-150 st-pb-base-60"
      ...
      <header className="st-flex st-items-center st-justify-between st-gap-base max-[600px]:st-flex-col max-[600px]:st-items-stretch">
        <div className="st-flex st-flex-1 st-items-end st-gap-base max-[600px]:st-flex-col max-[600px]:st-items-stretch">
          <TextInput
            ...
            path="seo-schema-name"
            readOnly={structuralLocked || readOnly}
            value={draft.name}
          />
          {collectionTemplate ? (
            <CheckboxInput
              checked={draft.isDefault === true}
              className="st-pb-base-35"
              label={t('defaultSchema')}
              name="seo-schema-default"
              onToggle={(event: ChangeEvent<HTMLInputElement>) =>
                onChange({ ...draft, isDefault: event.target.checked })
              }
              readOnly={structuralLocked || readOnly}
            />
          ) : null}
      ...
      {showLocalizedNotice ? (
        <Banner>{t('localizedEditingNotice')}</Banner>
      ) : null}
      <RecursiveSchemaEditor
        baseSchema={baseDraft?.schema}
        onChange={(schema) => onChange({ ...draft, schema })}
        readOnly={readOnly}
        schema={draft.schema}
      ...
      <RawSchemaEditor
        baseSchema={baseDraft?.schema}
        onApply={(schema) => onChange({ ...draft, schema })}
        readOnly={readOnly}
        schema={draft.schema}
        structuralLocked={structuralLocked}
      />

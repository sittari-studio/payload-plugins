...
export const isLocalizedSchemaLocale = ({
  defaultLocale,
  locale,
  localization,
}: {
  ...
  Boolean(localization && defaultLocale && locale && locale !== defaultLocale);
...
export const resolveCollectionLabel = (
  ...
  const matchingKey = Object.keys(label).find((key) => {
    ...
    return (
      normalizedKey === normalizedLanguage || normalizedKey === baseLanguage
    );
  });

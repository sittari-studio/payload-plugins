...
const same = (left: JsonValue | undefined, right: JsonValue | undefined) =>
...
const ScalarEditor = ({
  ...
  const overridden =
    locked && baseValue !== undefined && !same(baseValue, value);
  ...
  if (typeof value === 'boolean')
    return (
      <div className="st-flex st-h-full st-min-h-0 st-min-w-0 st-items-stretch st-gap-1.5">
        <CheckboxInput
          checked={value}
  ...
  if (typeof value === 'number')
    return (
      <div className="seo-schema-compact-field st-flex st-h-full st-min-h-0 st-min-w-0 st-items-stretch st-gap-1.5">
        <TextInput
          onChange={(event: ChangeEvent<HTMLInputElement>) =>
  ...
  if (value === null)
    return (
      <div className="st-flex st-h-full st-min-h-0 st-min-w-0 st-items-stretch st-gap-1.5">
        <Pill className="st-font-mono" pillStyle="light-gray">
          null
        </Pill>
        {controls}
      </div>
  ...
  return (
    <div className="st-flex st-h-full st-min-h-0 st-min-w-0 st-items-stretch st-gap-1.5">
      <VariableInput
        disabled={readOnly}
        onChange={onChange}
        onFocus={() => setEditingInherited(true)}
        path={path}
        value={String(value)}
        variables={variables}
      />
...
const TypeMenu = ({
  ...
  close: () => void;
  onChoose: (type: SchemaValueType) => void;
}) => {
  const t = useAdminText();
  return (
...
const ContainerEditor = ({
  ...
  const t = useAdminText();
  const entries: Array<[number | string, JsonValue]> = Array.isArray(value)
  ...
  const add = (type: SchemaValueType) => {
    ...
    onChange(
      Array.isArray(value)
        ? [...value, child]
        : { ...value, [uniquePropertyName(value)]: child },
    );
  };
  return (
    <div className="st-grid st-gap-base-50">
      <div
        className="st-font-mono st-text-[11px] st-text-elevation-550"
      ...
      <div className="st-grid st-gap-base-40">
        {entries.map(([key, child], index) => {
          ...
          return (
            <div
              className="st-grid st-gap-base-25"
            ...
            >
                ...
                <div className="st-grid st-h-full st-min-h-0 st-min-w-0 st-grid-cols-[minmax(0,.35fr)_minmax(0,.2fr)_minmax(0,1fr)_auto] st-items-stretch st-gap-2 [&>*]:st-min-h-0 [&>*]:st-min-w-0">
                  {Array.isArray(value) ? (
                    ...
                    <PropertyNameInput
                      ...
                      name={String(key)}
                      onCommit={(name) =>
                        onChange(renameSchemaProperty(value, String(key), name))
                      }
                      path={`seo-schema-property-${breadcrumb.join('-')}-${String(key)}`}
                    />
                  )}
                  <SelectInput
                    className="seo-schema-select st-h-full st-min-h-0 st-min-w-0"
                    isClearable={false}
                    name={`schema-type-${breadcrumb.join('-')}-${String(key)}`}
                    onChange={(option) => {
                      if (
                        ...
                        replaceChild(
                          key,
                          createSchemaValue(option.value as SchemaValueType),
                        );
                    }}
                    ...
                    path={`schema-type-${breadcrumb.join('-')}-${String(key)}`}
                    readOnly={locked || readOnly}
                    value={childType}
                  />
                  {nested ? (
                    <span className="st-py-2 st-text-elevation-600">
                      {Array.isArray(child)
                        ? t('itemsCount', { count: String(child.length) })
                        : t('propertiesCount', {
                            count: String(
                              Object.keys(child as JsonObject).length,
                            ),
                          })}
                    </span>
                  ) : (
                    <ScalarEditor
                      baseValue={childAt(baseValue, key)}
                      locked={locked}
                      onChange={(next) => replaceChild(key, next)}
                      path={`seo-schema-value-${breadcrumb.join('-')}-${String(key)}`}
                      readOnly={readOnly}
                      value={child}
                      variables={variables}
                    />
              ...
              {nested ? (
                <Collapsible
                  className="st-ml-[25px]"
                  header={t(childType === 'array' ? 'array' : 'object')}
                  initCollapsed={false}
                >
                  <ContainerEditor
                    baseValue={childAt(baseValue, key)}
                    breadcrumb={[...breadcrumb, String(key)]}
                    locked={locked}
                    onChange={(next) => replaceChild(key, next)}
                    readOnly={readOnly}
                    value={child as JsonObject | JsonValue[]}
                    variables={variables}
                  />
                </Collapsible>
              ) : null}
      ...
      {!locked && !readOnly ? (
        <Popup
          button={`+ ${Array.isArray(value) ? t('addItem') : t('addProperty')}`}
          buttonClassName="btn btn--style-dashed btn--size-small btn--no-margin"
          buttonType="default"
          caret={false}
          render={({ close }) => <TypeMenu close={close} onChoose={add} />}
          size="fit-content"
        />
      ) : locked ? (
        <Banner>{t('structureInherited')}</Banner>
      ) : null}
    </div>
  );
};

export const RecursiveSchemaEditor = ({
  baseSchema,
  onChange,
  readOnly,
  schema,
  ...
  <ContainerEditor
    ...
    onChange={(value) => onChange(value as JsonObject)}
    readOnly={readOnly}
    value={schema}
    variables={variables}
  />

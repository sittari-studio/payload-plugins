...
export const VariableInput = ({
  ...
  const matches = useMemo(() => {
    ...
    return variables
      .filter((variable) =>
        `${variable.path} ${variable.label} ${variable.collection}`
          .toLowerCase()
          .includes(normalized),
      )
      .slice(0, 12);
  ...
  const choose = (variable: SeoSchemaVariable) => {
    ...
    if (!input) return;
    const result = insertVariableAtCaret(
      value,
      variable.path,
  ...
  return (
    <div
      ...
      onBlur={(event) => {
        if (event.target instanceof HTMLInputElement) setQuery(undefined);
      }}
      onClick={(event) => {
        if (event.target instanceof HTMLInputElement) updateQuery(event.target);
      }}
      onFocus={(event) => {
    ...
    >
      ...
      {matches.length ? (
        <div
          className="st-absolute st-left-0 st-right-0 st-top-[calc(100%+4px)] st-z-30 st-grid st-max-h-[260px] st-min-w-[180px] st-overflow-auto st-rounded-sm st-border st-border-solid st-border-elevation-200 st-bg-elevation-0 st-shadow-popover"
          id="seo-schema-variable-list"
          role="listbox"
        >
          {matches.map((variable, index) => (
            <Button
              ...
              key={`${variable.collection}:${variable.path}`}
              margin={false}
            ...
            >
              <span className="st-grid st-gap-0.5">
                ...
                <code className="st-text-[11px] st-text-blue-600">
                  ${variable.path}
                </code>
                <small className="st-text-elevation-500">
                  {variable.collection}
                  {variable.availableInEveryCollection === false
                    ? ` · ${t('notEveryCollection')}`
                    : ''}
                </small>
              </span>
            </Button>

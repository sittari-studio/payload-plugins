...
describe('dev RBAC fixture', () => {
  it('seeds protected and editable example roles', async () => {
    ...
    const roles = result.docs.map(({ name, permissions }) => ({ name, permissions }))

    expect(roles).toEqual([
      {
        name: 'Content Editor',
        permissions: ['*:read', 'pages:create', 'pages:update', 'media:create', 'media:update'],
      },
      { name: devAdminRoleName, permissions: ['*'] },
  ...
  it('registers the localized permissions matrix client component', () => {
    const rolesCollection = payload.config.collections?.find(({ slug }) => slug === 'roles')
    const permissions = rolesCollection?.fields.find(
      (field): field is SelectField =>
        'name' in field && field.name === 'permissions' && field.type === 'select',
    )
    ...
    const rows =
      fieldComponent && typeof fieldComponent === 'object' && 'clientProps' in fieldComponent
        ? (fieldComponent.clientProps as {
            rows?: Array<{ label: Record<string, string> | string; slug: string }>
          }).rows
        : undefined
